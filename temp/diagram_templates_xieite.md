# Mermaid Diagram Style Guide for XIEITE Documentation

## Theme Configuration

All diagrams MUST include this configuration block:
...
config:
    ...
    themeVariables:
        ...
        fontSize: '14px'
        labelBackground: '#262B33'
        textColor: '#C1C4CA'
        noteBkgColor: '#3a3f47ff'
        noteTextColor: '#C1C4CA'
        noteBorderColor: '#6a6f77ff'
        actorBorder: '#779DC9ff'
        actorBkg: '#2b4268ff'
        actorTextColor: '#C1C4CA'
        actorLineColor: '#C1C4CAAA'
        signalColor: '#C1C4CA'
        signalTextColor: '#C1C4CA'
        labelBoxBkgColor: '#262B33'
        labelBoxBorderColor: '#3a3f47ff'
        labelTextColor: '#C1C4CA'
        loopTextColor: '#C1C4CA'
        activationBorderColor: '#779DC9ff'
        activationBkgColor: '#2b4268ff'
        sequenceNumberColor: '#262B33'
---
```
...
- **Process**: Blue (`#2b4268ff`)
- **Decision**: Purple (`#4d4962ff`)
- **Data/Storage**: Gray (`#3a3f47ff`)
- **Special Process**: Brown (`#7a6253ff`)
- **Success/End**: Green (`#425f5fff`)
- **Error/Failure**: Red (`#724848ff`)
- **Warning/Caution**: Yellow (`#7a7253ff`)

### Style Declaration Syntax

```
style NODE_ID fill:#COLORff,stroke:#STROKECOLORff,stroke-width:2px,color:#C1C4CA,rx:8,ry:8
```
...
flowchart TD
    ...
    Check -->|True| Process[Process Data]
    Check -->|False| Error[Handle Error]
    Process --> Store[(Store Result)]
    Store --> End([End])
    Error --> End

    style Start fill:#425f5fff,stroke:#8c9c81ff,stroke-width:2px,color:#C1C4CA,rx:8,ry:8
    style Check fill:#4d4962ff,stroke:#8983a5ff,stroke-width:2px,color:#C1C4CA,rx:8,ry:8
    style Process fill:#2b4268ff,stroke:#779DC9ff,stroke-width:2px,color:#C1C4CA,rx:8,ry:8
    style Error fill:#724848ff,stroke:#ac9696ff,stroke-width:2px,color:#C1C4CA,rx:8,ry:8
    style Store fill:#3a3f47ff,stroke:#6a6f77ff,stroke-width:2px,color:#C1C4CA,rx:8,ry:8
    style End fill:#425f5fff,stroke:#8c9c81ff,stroke-width:2px,color:#C1C4CA,rx:8,ry:8
```
## Flow Chart (Simple)
...
## Flow Chart (Complex Flow with Subgraphs)
...
1. **ALWAYS** include the full theme configuration block
2. **ALWAYS** use the exact color codes with `ff` suffix
3. **NEVER** add spaces after style property names
4. **ALWAYS** use semantic colors for node types
5. **MAINTAIN** consistent stroke-width of `2px`
6. **USE** monospace font family
7. **APPLY** corner radius of `rx:8,ry:8` for rounded nodes
8. **ENSURE** text color is `#C1C4CA` for all nodes
