...
namespace xieite::containers {
	...
	public:
		...
		XIEITE_ARROW_BASE(xieite::functors::unroll<RestKeys...>([&self, &keys]<std::size_t... i> XIEITE_ARROW(std::forward_like<Self>(self.map.at(std::get<0>(keys))[std::make_tuple(std::get<i + 1>(keys)...)]))))
