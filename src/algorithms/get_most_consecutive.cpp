...
export namespace xieite::algorithms {
    template <std::ranges::forward_range Range, /*xieite::concepts::Invocable<bool(std::ranges::range_reference_t<Range>)>*/ typename Functor>
        requires(std::ranges::sized_range<Range>)
    [[nodiscard]] constexpr std::ranges::subrange<std::ranges::iterator_t<Range>> getMostConsecutive(Range& range, Functor&& selector = Functor()) noexcept(xieite::concepts::NoThrowInvocable<Functor, bool(std::ranges::range_reference_t<Range>)> && xieite::concepts::NoThrowOperableRange<Range>) {
