...
export namespace xieite::functors {
    ...
    public:
        ...
        explicit(false) constexpr Function(Functor&& functor) noexcept
            : pointer(std::make_unique<Function::Derived<std::remove_cvref_t<Functor>>>(std::forward<decltype(functor)>(functor))) {
        }
        ...
        /* discardable */ constexpr Return operator()(ArgumentReferences&&... arguments) const noexcept(false) {
            ...
            return (*this->pointer)(std::forward<decltype(arguments)>(arguments)...);
        }
    ...
    private:
        ...
        struct Derived
            ...
            explicit(false) constexpr Derived(FunctorReference&& functor) noexcept
                : functor(std::forward<decltype(functor)>(functor)) {
            }

            /* discardable */ constexpr Return operator()(Arguments... arguments) const noexcept(false) override {
                return std::invoke(this->functor, std::forward<decltype(arguments)>(arguments)...);
            }
