...
export namespace xieite::types {
    ...
    public:
        ...
        constexpr static bool all = xieite::concepts::SatisfiedByAll<condition, Types...>;

        template <auto condition>
        constexpr static bool any = xieite::concepts::SatisfiedByAny<condition, Types...>;

        template <typename Type, auto comparator = []<typename First, std::same_as<First>> {
        }>
        constexpr static bool has = (... || xieite::concepts::SatisfiedBy<comparator, Type, Types>);
        ...
        template <typename Type, auto comparator = []<typename First, std::same_as<First>> {
            ...
            requires(List::has<Type, comparator>)
                    constexpr static std::size_t get = List::find<[]<typename OtherType>
                                                                      requires(xieite::concepts::SatisfiedBy<comparator, Type, OtherType>)
                    {
                    }>;

        template <std::size_t index>
            requires(index < sizeof...(Types))
        using At = std::type_identity_t<decltype(std::get<index>(std::tuple<Types...>{}))>;

        constexpr static auto apply(auto&& callback)
            XIEITE_ARROW(std::forward<decletype(callback)>(callback).template operator()<Types...>())

                template <auto validator>
                constexpr static bool satisfies = xieite::concepts::SatisfiedBy<validator, Types...>;
    ...
    public:
        template <auto selector>
        using Filter = decltype((List::TransformHelper<selector, []<typename Type> {
                                     return std::type_identity<Type>();
                                 }>()
                                     ->*...->*std::type_identity<Types>()))::type;

        template <auto comparator = []<typename First, typename... Rest>
        ...
        using TransformFlat = decltype(xieite::functors::unroll<sizeof...(Types) / arity>([]<std::size_t... i> {
            return (List::TransformHelper<[]<typename...> {
                    },
